...
- The bundled `sulogin` and `watchdogd` are now built and installed by
  default, like keventd and the D-Bus support.  `--without-sulogin` and
  `--without-watchdog` opt out.  Note, Finit starts `watchdogd` by itself
  when the WDT device node exists
